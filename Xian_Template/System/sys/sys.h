...
*********************************************************************************************************
*/
#include <stdarg.h>					/* C库文件，参数个数未知时获取函数中的参敿*/
#include <stdio.h>					/* C库文件，标准输入输出函数 */
#include <stdlib.h>					/* C库文件，定义亿个变量类型，一些宏和各种通用工具函数 */
#include <math.h>					/* C库文件，各种数学函数 */
#include <string.h>					/* C库文件，各种操作字符数组的函敿*/
/*
*********************************************************************************************************
*                                           ST HAL庿
*********************************************************************************************************
...
/* 方便RTOS里面使用 */
extern void SysTick_ISR(void);		/* 滴答定时器中断外部文件声昿*/

#define bsp_ProPer1ms  SysTick_ISR

//位带操作,实现51类似的GPIO控制功能
//具体实现思想,参耿<CM3权威指南>>第五竿87页~92顿.M4同M3类似,只是寄存器地址变了.
//IO口操作宏定义
...
void INTX_DISABLE(void);//关闭所有中斿
void INTX_ENABLE(void);	//开启所有中斿
...
/* 用于设置是否关闭栈填充，默认情况下是使能的，所有任务的栈空间全部填充为0xEF＿
*  带有ThreadX调试组件或者运行时栈检测会用到?
*/
//#define TX_DISABLE_STACK_FILLING


/* 用于使能栈检测，默认是关闭的。此选项使能后，而TX_DISABLE_STACK_FILLING没使能时，栈填充将开启，方便栈检浿*/
...
/* 使能tx_thread_resume和tx_thread_suspend使用内联代码，优势是提升这两个函数的执行性能，劣势是增加代码釿*/
//#define TX_INLINE_THREAD_RESUME_SUSPEND


/* 设置TreadX内核不可中断，好处是降低处理负担，并且产生的代码小。但增加锁时闿*/
//#define TX_NOT_INTERRUPTABLE
