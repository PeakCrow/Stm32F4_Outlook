...
{
    /* 定义并创建图像按钮 */
    lv_obj_t *Imgbtn_MC = lv_imgbtn_create(parent);
    /* 设置按钮释放时的图像 */
	lv_img_set_src(Imgbtn_MC,"0:/PICTURE/dm_motor.bin");
    //lv_imgbtn_set_src(Imgbtn_MC,LV_STATE_DEFAULT,NULL,"0:/PICTURE/dm_motor.bin",NULL);//pack.png  motor_control.png
    /* 设置按钮大小 */
    lv_obj_set_size(Imgbtn_MC,150,150);
    /* 设置按钮位置 */
    //lv_obj_align_to(Imgbtn_MC,parent,LV_ALIGN_TOP_RIGHT,-(lv_obj_get_x2(parent))*1/15,0);
		lv_obj_set_pos(Imgbtn_MC,50,280);
//    /* 普通样式将透明度设置为50 */
