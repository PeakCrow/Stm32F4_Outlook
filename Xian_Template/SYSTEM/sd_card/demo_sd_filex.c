		...
		if (comGetChar(COM1, &cmd))	/* 从串口读入一个字符(非阻塞方式) */
		{
			...
			switch (cmd)
			{
				...
				case '3':
					...
					ReadFileData();		/* 读取根目录下armfly.csv的内容 */
					break;
...
static void DispMenu(void)
{
	...
	printf("2 - 创建一个新文件armfly.csv\r\n");
	printf("3 - 读armfly.csv文件的内容\r\n");
	printf("4 - 创建目录\r\n");
	printf("5 - 删除文件和目录\r\n");
	printf("6 - 读写文件速度测试\r\n");
}
...
static void CreateNewFile(void)
{
    ...
    if (status == FX_SUCCESS)
    {
        printf("armfly.csv 文件写入成功\r\n");        
    }
    else
    {
        printf("armfly.csv 文件写入失败 %d\r\n", status); 
    }
...
*********************************************************************************************************
*	函 数 名: ReadFileData
*	功能说明: 读取文件armfly.csv前128个字符，并打印到串口
*	形    参：无
*	返 回 值: 无
*********************************************************************************************************
*/
...
*********************************************************************************************************
*	函 数 名: DeleteDirFile
*	功能说明: 删除SD卡根目录下的 armfly.csv 文件和 Dir1，Dir2 目录
*	形    参：无
*	返 回 值: 无
*********************************************************************************************************
*/
static void DeleteDirFile(void)
{
	...
	{
		printf("删除armfly.csv失败(错误代码 = %d) 文件只读或目录非空\r\n", status);
	}   
