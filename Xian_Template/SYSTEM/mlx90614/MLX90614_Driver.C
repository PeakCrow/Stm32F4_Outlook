...
#define SMBUS_SCK		GPIO_PIN_4 //PB10：SCL
#define SMBUS_SDA		GPIO_PIN_5 //PB11：SDA

#define MLX90614_GPIO_CLK_ENABLE()   __HAL_RCC_GPIOC_CLK_ENABLE()
...
void SMBus_Init()
{
	...
	HAL_GPIO_Init(SMBUS_PORT,&GPIO_InitStruct);                    //初始化PB 10/11
...
{   
	float temp;
	uint8_t rx_buffer[2];
	uint8_t buf[1] = {(RAM_ACCESS|RAM_TOBJ1)};//0x07
	
	I2C_EE_ByteWrite(buf,(SA<<1));//0x00
	I2C_EE_ByteWrite(0,0x01);
	I2C_EE_BufferRead(rx_buffer,((SA<<1)+1),2);//0x01
