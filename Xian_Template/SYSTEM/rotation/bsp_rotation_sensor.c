...
void bsp_InitRotationSensor(void)
{
	...
	g_RotationTimeHandle.Init.Period 			= ROTATION_SENSOR_TIM_PERIOD;		/* 定时器2是32位定时器 */
	...
	__HAL_TIM_ENABLE_IT(&g_RotationTimeHandle, TIM_IT_UPDATE);						/* 使能定时器溢出更新中断 */
	...
	HAL_NVIC_EnableIRQ(TIM2_IRQn);				/* 配置中断优先级  */
...
{
	(void)_htim;
	g_updatecounter++;							/* 定时器每溢出依次，计数器加1 */
}

/*******************************************************************************
  * @FunctionName: HAL_TIM_Base_MspInit
  * @Author:       trx
  * @DateTime:     2022年6月15日 22:14:07 
...
/*******************************************************************************
  ...
  * @Author:       trx
  * @DateTime:     2022年6月15日 22:05:34 
  * @Purpose:      轮速计算,0.5s采集计算一次
  * @param:        _gear_num    ：车轮齿数
...
float Rotation_Sensor_Get(uint8_t _gear_num,float _wheel_radius)
{
	...
	float rotation = 0;
	pulse_num =(__HAL_TIM_GET_COUNTER(&g_RotationTimeHandle) + (g_updatecounter * ROTATION_SENSOR_TIM_PERIOD));/* 得到脉冲数值 */
	/* 在这里得到的脉冲是 含有上升沿和下降沿，但是在除以0.5s的时间后正好去掉了一半脉冲 */
