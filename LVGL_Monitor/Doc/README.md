...
=======
## 24.GCC编译器下 threadx 一直无法正常启动？

- [x] 正确将各个段的数据进行搬运后可以正常启动，其中向量表段名从.vectors改为._vectors，无法正常启动，因为此时会造成向量表被丢弃或放错位置，链接选项有--start-group改为-start-group，会造成链接文件无法正确处理库分组，同样也会造成OS无法正常启动
