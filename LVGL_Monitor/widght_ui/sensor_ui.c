...
void Sensor_Ui(lv_obj_t *parent)
{
    ...
    lv_imgbtn_set_src(Imgbtn_MC,LV_STATE_DEFAULT,NULL,"D:/Ls_Monitor_Lower/LVGL_Monitor/images/sensor.png","D:/Ls_Monitor_Lower/LVGL_Monitor/images/sensor.png");
#else
...
static void Imgbtn_MC_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_RELEASED)
         Sensor_In_Ui(App_Common_Init("Sensor",App_btn_Back_Cb));
}

/*************************应用代码************************************************/
/* 图表回调函数，使用label来显示按下数据点的数值 */
...
static void Sensor_In_Ui(lv_obj_t * parent)
{
    ...
    Realtime_Sensor_timer = lv_timer_create(Realtime_Sensor_Cb,100,NULL);
